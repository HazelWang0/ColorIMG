...
class ImageLogger(Callback):
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        ...
        if (self.check_frequency(check_idx) and  # batch_idx % self.batch_freq == 0
            ...
            json_dir = os.path.join(pl_module.logger.save_dir, 'images', 'val', 'caption.json')
            ...
            for k in images:
                """ my add token record"""
                if isinstance(images[k], list):
                    for i in images[k]:
                        exist_json.append(i)
                    with open(json_dir, 'w') as f:
                        json.dump(exist_json, f, indent=4)
                    continue
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx):
        if not self.disabled and (pl_module.global_step > 0 or self.log_first_step):
            self.log_img(pl_module, batch, batch_idx, split="train") # 显示图片
...
class CUDACallback(Callback):
    ...
    def on_train_epoch_end(self, trainer, data):
        torch.cuda.synchronize(trainer.root_gpu)
...
if __name__ == "__main__":
    ...
    # model
    model = instantiate_from_config(config.model) # AutoencoderKLResi
    ...
    os.makedirs(os.path.join(logdir, 'testtube'), exist_ok=True)
