...
from basicsr.data.coco_dataloader import COCODataset 
...
def main():
    ...
    # data
    dataset = COCODataset(config.test_data.params.test.params)
            ...
            with model.ema_scope():
                ...
                for batch_idx, samples in enumerate(test_dataloader):
                    if os.path.exists(os.path.join(latent_path, f"{base_i:08}.npy")):
                        ...
                        # x_T = None
                        '''sc torch.Size([1, 77, 1024]) inpl torch.Size([1, 4, 64, 64]) t 200 xt torch.Size([1, 4, 64, 64])'''
                        samples, _ = model_ori.sample(cond=semantic_c, struct_cond=init_latent, batch_size=init_image.size(0), timesteps=opt.ddpm_steps, time_replace=opt.ddpm_steps, return_intermediates=True)
                        samples, _ = model.sample(cond=semantic_c, struct_cond=init_latent, batch_size=init_image.size(0), timesteps=1000, time_replace=1000, return_intermediates=True)
                        samples, _ = model.sample(cond=semantic_c, struct_cond=init_latent, batch_size=init_image.size(0), timesteps=opt.ddpm_steps, time_replace=opt.ddpm_steps, return_intermediates=True)
                        ...
                        # ===
                        samples, _ = self.sample(cond=c, struct_cond=struct_cond, batch_size=N, timesteps=cur_time_step, return_intermediates=True, time_replace=self.time_replace)
                        
                        x_samples = model.decode_first_stage(samples)
                        x_samples = torch.clamp((x_samples + 1.0) / 2.0, min=0.0, max=1.0) # torch.Size([1, 3, 512, 512])
