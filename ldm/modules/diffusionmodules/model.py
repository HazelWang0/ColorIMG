...
class Decoder_Mix(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            if i_level != self.num_resolutions-1:  # CFW阶段
                if i_level != 0:
    ...
    def forward(self, z, enc_fea):
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            if i_level != self.num_resolutions-1 and i_level != 0:
                ...
                h = cur_fuse_layer(enc_fea[i_level-1], h, self.fusion_w)  # call ldm/modules/diffusionmodules/model.py Fuse_sft_block_RRDB
        ...
        if self.tanh_out:
            h = torch.tanh(h)
        return h # torch.Size([1, 3, 512, 512])
