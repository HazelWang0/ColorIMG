...
class AutoencoderKLResi(pl.LightningModule):
    ...
    def encode(self, x):
        ...
        moments = self.quant_conv(h) # torch.Size([1, 8, 64, 64])
        posterior = DiagonalGaussianDistribution(moments)
        # posterior = h
    ...
    def forward(self, input, latent, sample_posterior=True):
        posterior, enc_fea_lq = self.encode(input) # input torch.Size([1, 3, 512, 512])
        dec = self.decode(latent, enc_fea_lq) # katent shape torch.Size([1, 4, 64, 64])
        return dec, posterior
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 0:
            # train encoder+decoder+logvar
            aeloss, log_dict_ae = self.loss(gts, reconstructions, posterior, optimizer_idx, self.global_step,  # call LPIPSWithDiscriminator
                                            last_layer=self.get_last_layer(), split="train")
            self.log("aeloss", aeloss, prog_bar=True, logger=True, on_step=True, on_epoch=True)
            self.log_dict(log_dict_ae, prog_bar=False, logger=True, on_step=True, on_epoch=False)
            return aeloss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        reconstructions, posterior = self(inputs, latents)
        aeloss, log_dict_ae = self.loss(gts, reconstructions, posterior, 0, self.global_step,  # LPIPSWithDiscriminator
                                        last_layer=self.get_last_layer(), split="val")
