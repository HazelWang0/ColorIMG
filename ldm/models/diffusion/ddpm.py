...
from basicsr.data.transforms import paired_random_crop, random_crop, triplet_random_crop
...
# for mask bool type
#mask =  (255*masks[0, :, :, None] + cur_img*0.).astype(np.uint8)
# if mask tensor bool 
#mask = mask.float()
...
def save_img(img, name, itr = 0, root='_tempt/seg_debug/'):
    pathlib.Path(root).mkdir(parents=True, exist_ok=True) 
    path = os.path.join(root, name+"_{}.png".format(itr))
    if img.dtype == torch.bool:
        save_image(img.float(), path)
        return
    if isinstance(img, np.ndarray):
        # to8b = lambda x : (255*np.clip(x,0,1)).astype(np.uint8)
        # img = to8b(img)
        pil_feedback_image = Image.fromarray(img)
        pil_feedback_image.save(path)
        return 
    if ((img.dim() == 3 and img.shape[0] == 1) or (img.dim() == 4 and img.shape[1] == 1)):
        img = np.array(img.squeeze().detach().cpu())
        img = cv2.applyColorMap(cv2.convertScaleAbs(img,alpha=15),cv2.COLORMAP_JET)
        pil_feedback_image = Image.fromarray(img)
        pil_feedback_image.save(path)
...
class LatentDiffusionSRTextWTCOCO(DDPM):
    """main class"""
    def __init__(self,
                 first_stage_config,
                 cond_stage_config,
    ...
    def instantiate_cond_stage(self, config):
        if not self.cond_stage_trainable:
            if config == "__is_first_stage__":
                print("Using first stage also as cond stage.")
                self.cond_stage_model = self.first_stage_model # FrozenOpenCLIPEmbedder
            elif config == "__is_unconditional__":
                print(f"Training {self.__class__.__name__} as an unconditional model.")
                self.cond_stage_model = None
                # self.be_unconditional = True
    ...
    def randn_cropinput(self, gt, base_size=[64, 128, 256, 512]):
    ...
    def get_sp(self, img, mask, gt):
        slic = cv2.ximgproc.createSuperpixelSLIC(img, ruler = 20.0)
        slic.iterate(5)
        ...
        x_sp = torch.from_numpy(quantize_img).permute(2,0,1)
    ...
    def get_input(self, batch, k=None, return_first_stage_outputs=False, force_c_encode=False,
        ...
        # clamp and round
        im_lq = torch.clamp(out, 0, 1.0)  # torch.Size([1, 3, 512, 512])

        # random crop
        gt_size = self.configs.degradation['gt_size']
        im_gt = random_crop(im_gt, gt_size, self.configs.sf)
        self.gt = im_gt
        ...
        if self.random_size:
            self.gt = self.randn_cropinput(self.lq, self.gt)
        ...
        # x为黑白图
        # x = batch['hint']*2 -1 # my change
        # x = batch['hint'] # my change
    ...
    def apply_model(self, x_noisy, t, cond, struct_cond, return_ids=False):
        ...
        if isinstance(cond, dict):
            ...
            if not isinstance(cond, list): # into
                cond = [cond] 
            key = 'c_concat' if self.model.conditioning_key == 'concat' else 'c_crossattn'
            cond = {key: cond}
        ...
        if hasattr(self, "split_input_params"):
            ...
            # apply model by loop over crops
            output_list = [self.model(z_list[i], t, **cond_list[i]) for i in range(z.shape[-1])] # 重点 DiffusionWrapper
            assert not isinstance(output_list[0],
                                  tuple)  # todo cant deal with multiple model outputs check this never happens
            ...
            # x_noisy = torch.ones_like(x_noisy) / 2
            x_recon = self.model(x_noisy, t, **cond)
    ...
    def p_losses(self, x_start, cond, struct_cond, t, t_ori, z_gt, noise=None): # key
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)

        if self.mix_ratio > 0: # jump
            if random.random() < self.mix_ratio:
                noise_new = default(noise, lambda: torch.randn_like(x_start))
                noise = noise_new * 0.5 + noise * 0.5
                x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
                
        model_output = self.apply_model(x_noisy, t_ori, cond, struct_cond) # key
        ...
        if self.parameterization == "x0":
            target = x_start
        elif self.parameterization == "eps": # into
            target = noise
        elif self.parameterization == "v":
            target = self.get_v(x_start, noise, t)
        else:
        ...
        loss_simple = self.get_loss(model_output_, target, mean=False).mean([1, 2, 3])  # target 是 noise
        loss_dict.update({f'{prefix}/loss_simple': loss_simple.mean()})
    ...
    def p_mean_variance(self, x, c, struct_cond, t, clip_denoised: bool, return_codebook_ids=False, quantize_denoised=False,
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.: # into
        ...
        if self.parameterization == "eps": # into
            x_recon = self.predict_start_from_noise(x, t=t, noise=model_out)
        elif self.parameterization == "x0":
            x_recon = model_out
        elif self.parameterization == "v":
        ...
        if reference_sr is not None: # jump
    ...
    def p_sample_loop(self, cond, struct_cond, shape, return_intermediates=False,
        ...
        for i in iterator:
            ...
            if self.shorten_cond_schedule: # in
                assert self.model.conditioning_key != 'hybrid'
                tc = self.cond_ids[ts].to(cond.device)
                cond = self.q_sample(x_start=cond, t=tc, noise=torch.randn_like(cond))
            ...
            if t_replace is not None:
                if start_T is not None:
                    if self.ori_timesteps[i] > start_T:
                         continue
                struct_cond_input = self.structcond_stage_model(struct_cond, t_replace) # EncoderUNetModelWT encoder的核心
            else: # in
                if start_T is not None:
                    if i > start_T:
                        continue
                struct_cond_input = self.structcond_stage_model(struct_cond, ts) # 这里不一样

            if interfea_path is not None: # ju
        ...
        if sample:
            ...
            with self.ema_scope("Plotting"):
                if self.time_replace is not None:
                    ...
                    cur_time_step = 1000
                ''''c torch.Size([4, 77, 1024]) str_c torch.Size([4, 4, 64, 64]) struct_cond = z torch.Size([4, 4, 64, 64]) t1000'''
            ...
            log["samples"] = x_samples
            # import torchvision
            # save_img(x_samples,f'{text_cond[0]}')
            # save_img(x_samples,f'{text_cond}')
...
class DiffusionWrapper(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key):
        super().__init__()
        self.diffusion_model = instantiate_from_config(diff_model_config) # UNetModelDualcondV2
        self.conditioning_key = conditioning_key
        assert self.conditioning_key in [None, 'concat', 'crossattn', 'hybrid', 'adm']

    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, struct_cond=None, seg_cond=None):
        if self.conditioning_key is None: # 'crossattn'  # into
            out = self.diffusion_model(x, t) # UNetModelDualcondV2  常规的unet
        elif self.conditioning_key == 'concat':
            xc = torch.cat([x] + c_concat, dim=1)
            out = self.diffusion_model(xc, t)
        elif self.conditioning_key == 'crossattn': # into
            cc = torch.cat(c_crossattn, 1)
            if seg_cond is None: # into  倒数第二层出现nan
